...
const SPAM_EXTENSIONS = new Set([".jpg", ".jpe", ".jpeg", ".png", ".gif"]);
...
const SPAM_GENERATORS = [randomChan, randomImg, randomDSC, randomImage];

function makeSpamName(ext) {
  return shuffle(SPAM_GENERATORS)[0](ext);
}

class Room extends VolaRoom {
  constructor(room_id, config, other) {
    config = config || {};
    const {user = null} = config;
    if (!user) {
      throw new Error("Invalid user");
    }
    super(room_id, user, other);
    this[CONFIG] = config;
    this.aborted = false;
    this.on("upload_blocked", to => {
  ...
  async upload(files, rate) {
    ...
    files = files.map(file => {
      ...
      if (spam && SPAM_EXTENSIONS.has(l_ext)) {
        return {file, name: makeSpamName(l_ext)};
      }
      if (prefix) {
    ...
    for (let attempt = 0; attempt < cAttempts; ++attempt) {
      ...
      try {
        const res = await super.uploadFile({
          ...
          file,
          highWaterMark,
          progress: (delta, current, length, server, resumed) => {
            rate.current += delta;
            bar().update(current, length, server, resumed);
            if (this.aborted) {
              throw new UserAbort("User aborted");
            }
          },
        });
        bar();
