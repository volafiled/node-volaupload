"use strict";

require("colors");
const fs = require("fs");
const path = require("path");
const log = require("loglevel");
const {promisify} = require("util");
const {Room: VolaRoom} = require("volapi");
const {filesize, ProgressBar, Rate} = require("./util");

const CONFIG = Symbol("config");

const DEFAULT_HIGHMARK_ORDER = 19;
const DEFAULT_HIGHMARK = 1 << DEFAULT_HIGHMARK_ORDER;

function makePrefix(cur, total) {
  total = `${total}`;
  return `[${cur.toString().lpad(total.length, "0")}/${total}] `;
}

class Room extends VolaRoom {
  constructor(roomid, config) {
    config = config || {};
    const {user = null} = config;
    if (!user) {
      throw new Error("Invalid user");
    }
    super(roomid, user);
    this[CONFIG] = config;
    this.aborted = false;
  }

  async upload(files) {
    const {passwd = null} = this[CONFIG];
    if (passwd) {
      log.info("Greenfagging in as".bold, this.nick.bold.green);
      await this.login(passwd);
    }
    log.info("Connecting to".bold, "mainframe".bold.yellow);
    await this.connect();

    const tlen = files.length === 1 ? "one" : files.length.toString();
    log.info(
      "Deploying".bold,
      tlen.bold.green,
      `botnet payload${tlen === "one" ? "" : "s"}`.bold.yellow);
    const {"delete-after": deleteAfter = false} = this[CONFIG];
    const rate = new Rate();
    const nameMax = files.reduce((p, c) => {
      const {base} = path.parse(c);
      return Math.max(p, base.length);
    }, 0);
    const total = files.length;
    let cur = 0;
    for (const file of files) {
      if (this.aborted) {
        break;
      }
      try {
        const prefix = total > 1 ? makePrefix(++cur, total) : "";
        await this.uploadFile(prefix, file, rate, nameMax);
        if (deleteAfter) {
          try {
            fs.unlinkSync(file);
          }
          catch (iex) {
            log.error(
              "Failed remove file".bold.red,
              `${file.bold}:`,
              iex.message || iex);
          }
        }
      }
      catch (ex) {
        log.error(
          "Failed to upload".bold.red,
          `${file.bold}:`,
          ex.message || ex);
      }
    }
    log.info(
      "All done in",
      `${rate.elapsed.toFixed(1)}s`.bold,
      `(${`${filesize(rate.rate)}/s`.cyan.blue})`);
    await this.close();
  }

  async uploadFile(prefix, file, rate, nameMax) {
    const {base} = path.parse(file);
    if (!base) {
      throw new Error("Not something I can upload");
    }
    const {highWaterMark = DEFAULT_HIGHMARK} = this.config;
    const stream = fs.createReadStream(file, {
      highWaterMark,
      encoding: null
    });
    const {size: total} = await promisify(fs.stat)(stream.path);
    const bar = new ProgressBar(prefix, base, { total, nameMax });

    class UserAbort extends Error {}

    try {
      await super.uploadFile(base, {
        stream,
        progress: (delta, current, length) => {
          rate.current += delta;
          bar.update(current, length);
          if (this.aborted) {
            throw new UserAbort();
          }
        },
      });
    }
    catch (ex) {
      bar.terminate(false);
      if (ex instanceof UserAbort) {
        return;
      }
      throw ex;
    }

    bar.terminate();
  }

  close() {
    this.aborted = true;
    return super.close();
  }
}

module.exports = { Room };
